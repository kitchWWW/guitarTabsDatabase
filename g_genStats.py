...
def analyze_songs_detailed(song_files):
    ...
    song_counts = {"verses": [], "choruses": [], "bridges": [], "prechoruses": [], "intros": [], "outros": []}
    ...
    for filepath in song_files:
        try:
            ...
            continue
        # Look inside the 'components' subfield for sections
        components = data.get('components', {})
        verses = {k: v for k, v in components.items() if k.lower().startswith('verse') and v}
        choruses = {k: v for k, v in components.items() if k.lower().startswith('chorus') and v}
    ...
    # Optionally persist the kept songs
    with open("kept_songs.txt", "w") as f:
        f.write(json.dumps(files_kept))

    # Process each valid song.
    for song in songs_kept:
        components = song.get('components', {})
        verses = {k: v for k, v in components.items() if k.lower().startswith('verse') and v}
        choruses = {k: v for k, v in components.items() if k.lower().startswith('chorus') and v}
        bridges = {k: v for k, v in components.items() if k.lower().startswith('bridge') and v}
        pre_choruses = {k: v for k, v in components.items() if k.lower().startswith('pre-chorus') and v}
        intros = {k: v for k, v in components.items() if k.lower().startswith('intro') and v}
        outros = {k: v for k, v in components.items() if k.lower().startswith('outro') and v}
        ...
        song_metrics['chords'].append(sum(len(chords) for section in [verses, choruses, bridges, pre_choruses, intros, outros] for chords in section.values()))
        song_metrics['unique'].append(len(set(song_all_chords)))
        song_metrics['chromatic'].append(compute_chromaticness(song_all_chords))
        # Shared unique chords between verses and choruses
        verse_unique = set().union(*verses.values())
        chorus_unique = set().union(*choruses.values())
        song_metrics['shared'].append(len(verse_unique.intersection(chorus_unique)))

    # Compile summary stats
    results = {
        'verse': { 'chords': compute_summary_stats(metrics['verse']['chords']),
                   'unique': compute_summary_stats(metrics['verse']['unique']),
                   'chromatic': compute_summary_stats(metrics['verse']['chromatic']) },
        'chorus':{ 'chords': compute_summary_stats(metrics['chorus']['chords']),
                   'unique': compute_summary_stats(metrics['chorus']['unique']),
                   'chromatic': compute_summary_stats(metrics['chorus']['chromatic']) },
        'bridge':{ 'chords': compute_summary_stats(metrics['bridge']['chords']),
                   'unique': compute_summary_stats(metrics['bridge']['unique']),
                   'chromatic': compute_summary_stats(metrics['bridge']['chromatic']) },
        'prechorus':{ 'chords': compute_summary_stats(metrics['prechorus']['chords']),
                      'unique': compute_summary_stats(metrics['prechorus']['unique']),
                      'chromatic': compute_summary_stats(metrics['prechorus']['chromatic']) },
        'intro': { 'chords': compute_summary_stats(metrics['intro']['chords']),
                   'unique': compute_summary_stats(metrics['intro']['unique']),
                   'chromatic': compute_summary_stats(metrics['intro']['chromatic']) },
        'outro': { 'chords': compute_summary_stats(metrics['outro']['chords']),
                   'unique': compute_summary_stats(metrics['outro']['unique']),
                   'chromatic': compute_summary_stats(metrics['outro']['chromatic']) },
        'song':  { 'chords': compute_summary_stats(song_metrics['chords']),
...
if __name__ == "__main__":
    song_files = glob.glob("out_structure/*.json")

...
output_dir = 'charts_transition_count'
...
for csv_path in glob.glob(os.path.join(input_dir, '*.csv')):
    ...
    heatmap = ax.imshow(df.values,
                        aspect='auto',
                        # vmin=0,       # minimum of the colormap
                        # vmax=maxNumb
                        )     # cap the colormap at 0.4
    ...
    # 6. Annotate each cell (shows actual % even if >0.4)
    threshold = df.max().max() / 2
    ...
    # 7. Colorbar (with extend so anything >0.4 is shown as “>0.4”)
